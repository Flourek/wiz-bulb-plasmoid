...
Item {
    ...
    // Helper function to get bulb state after successful operations
    function refreshBulbStateAfterOperation(operation, success, result) {
        operationCompleted(operation, success, result);
        if (success && isConnected) {
            console.log("[WizControl] Operation", operation, "successful, refreshing bulb state...");
            getBulbState();
        }
    }
            ...
            if (exitCode === 0 && stdout) {
                try {
                    ...
                    if (result.success && result.discovery && result.discovery.success) {
                        ...
                        // Set bulb IP from first discovered bulb
                        if (bulbCount > 0 && result.discovery.bulbs[0].ip) {
                            bulbIP = result.discovery.bulbs[0].ip;
                        }
    ...
    function _setBrightnessNow(brightness) {
        ...
        executeCommand("setBrightness", [brightness.toString()], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setBrightness", result.success, result);
                    if (!result.success) {
    ...
    function _setRGBColorNow(red, green, blue) {
        ...
        executeCommand("setRGB", [red.toString(), green.toString(), blue.toString()], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setRGB", result.success, result);
                    if (!result.success) {
    ...
    function setBulbPower(on) {
        ...
        executeCommand("setPower", [on ? "true" : "false"], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setPower", result.success, result);
                    if (!result.success) {
    ...
    function _setWarmWhiteNow(brightness, temperature) {
        ...
        executeCommand("setWarmWhite", [brightness.toString(), temperature.toString()], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setWarmWhite", result.success, result);
                    if (!result.success) {
    ...
    function setScene(sceneId) {
        ...
        executeCommand("setScene", [sceneId.toString()], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setScene", result.success, result);
                    if (!result.success) {
    ...
    function setSceneWithSpeed(sceneId, speed) {
        ...
        executeCommand("setSceneWithSpeed", [sceneId.toString(), speed.toString()], function(exitCode, stdout, stderr) {
            if (exitCode === 0 && stdout) {
                try {
                    const result = JSON.parse(stdout.trim());
                    refreshBulbStateAfterOperation("setSceneWithSpeed", result.success, result);
                    if (!result.success) {
